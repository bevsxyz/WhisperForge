...
- **`hf-hub` feature unification defeats core's sync-only optimization**: `whisperforge-core/Cargo.toml` declares `hf-hub = { version = "0.5", default-features = false, features = ["ureq"] }` (sync HTTP only); `whisperforge/Cargo.toml` declares `hf-hub = { version = "0.5", features = ["tokio"] }` (async). Cargo feature unification merges these to the binary's stronger requirement, so the final `wforge` binary compiles both `reqwest` (from tokio) and `ureq@3.3.0` — verified via `cargo tree -p whisperforge -e normal -i reqwest` (shows full dep chain) and `cargo tree -p whisperforge -e normal -i ureq@3.3.0` (both present). Core's HTTP stack optimization is overridden but harmless; the binary uses hf-hub's unified API, not the raw clients. This is a feature cost of hybrid async/sync, not a bug.
