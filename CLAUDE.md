...
**Known content gaps at the trim seam.** Even with force-commit, ~30% of mid-utterance content is dropped at trim boundaries — Whisper's non-causal encoder produces different tokens for the same audio across different buffer sizes, so LCP doesn't reliably confirm tokens that span a trim. The current implementation prefers structural continuity (one endpoint event per utterance, no duplicated text) over content fidelity. If a user needs every word, current behavior is `--max-window-secs 28` (no cap-hits on most utterances) or accepting multiple endpoint events per long utterance. Truly fixing this requires temperature-fallback sampling + working timestamps + timestamp-anchored trimming — a substantial follow-up.
