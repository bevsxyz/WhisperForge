...
**Live-mic perf ceiling — tiny.en + CPU is decode-bound, not encoder-bound.** Autoregressive decode is O(tokens × ~100 ms) on Burn's CPU backend, so per-window cost is dominated by however many tokens the model emits. WGPU is **worse** for this workload, not better: the encoder is comparable to CPU but the per-token dispatch tax pushes the autoregressive loop to ~3.5 s per window. **Defaults are tuned for CPU**: `--device cpu`, `stride_secs=1.0`, `max_window_secs=5.0`, hardcoded `max_new_tokens=32`. The 5 s cap bounds the decoder at ~25 tokens (≈ 2.5 s wall-clock) on real speech, comfortably under the 1 s stride; when an utterance exceeds 5 s the chunker forces an EOU at the cap (utterance splits into multiple endpoint events). This is a deliberate trade — a sliding-buffer alternative was prototyped and reverted because Whisper's non-causal encoder decodes overlapping audio differently across window sizes, breaking LocalAgreement-2 at the slide seam and producing duplicated text. Drops are surfaced on stderr (`[audio] dropped … samples last second …`) so silent loss is impossible; per-window mel/enc/dec latency is at `tracing::debug` (enable with `RUST_LOG=whisperforge=debug`). Long-form continuity (single committed line across the cap) requires either timestamp-anchored window trimming or a separately-trained block-causal encoder — both deferred until there's a demand signal.
